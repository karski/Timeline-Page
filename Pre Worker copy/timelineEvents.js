...
function Sortie(msnID, cocomID, eventAirfield, eventColor, squadron, schedStartTime, schedEndTime, missionNumber, groundStationNumber, tailNumber, sortieNote, actStartTime, actEndTime, msnEffective) {
    //if event is in the past or future, don't create it
    if ((pastTime != undefined && futureTime != undefined) &&
        ((schedEndTime < pastTime && actEndTime < pastTime) ||
            (schedStartTime > futureTime && actStartTime > futureTime))) {
        //do nothing - really shouldn't have gotten this far to begin with
    } else {
        //in the time limits (or time limits weren't passed), create the new event
        this.id = msnID;
        this.squad = squadron;
...
//TODO: consider checking if the station already exists?
