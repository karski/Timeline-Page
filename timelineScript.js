...
function changeRowMode() {
    //record selection for reloads
    let newMode = document.getElementById("selectRowMode").selectedOptions[0].value;
    addParamter("rowMode", newMode);
    updateTimelineRows(newMode);
    ...
    // TODO: implement changed row event without worker - try using existing events list to build out rows
}

//this function should be run on an interval to keep page info up to date as time and flight data changes
function updatePage() {
    changeRowMode(); // a little wasteful, but it will check for differences before actually redrawing
    updateTimeHeader();
    fetchAll();
